...
const createMeditationAudio = async (req, res) => {
  ...
  if (!title || /[/\\:*?"<>|]/.test(title)) {
    return res
      .status(400)
      .json({ error: "Invalid or missing title for MP3 filename." });
  }
  ...
  /// Check for duplicat title
  const meditationAudio = await MeditationAudio.find({ title });
  if (meditationAudio.length) {
    console.log(meditationAudio);
    return res.status(400).json({ error: "Title already existed!" });
  }

  let audioContent = "Hi PeacePod";
  try {
    audioContent = await generateMeditationSession({
      duration,
      mood,
      tone,
      extraNotes,
  ...
  try {
    ...
    const audioResponse = await axios.post(
      "http://localhost:4000/api/meditation/audios/text-to-audio",
      bodyAudioGeneration,
      {
    ...
    const cloudinaryResponse = await cloudinary.uploader.upload_stream(
      ...
      async (error, result) => {
        ...
        /// Create new meditation audio in DB
        const createdAudio = await MeditationAudio.create({
          userId,
          title,
          duration,
          mood,
...
/// Update isFavorite of a audio
const updateAudio = async (req, res) => {
  const { audioId } = req.params;
  const { isFavorite } = req.body;

  if (!audioId) return res.status(400).json({ error: "Audio id is required" });

  const updatedAudio = await MeditationAudio.findByIdAndUpdate(
    audioId,
    { isFavorite },
    { new: true }
  );
  ...
  if (!updatedAudio)
    return res
      .status(500)
      .json({ error: "Something happened while updating audio" });

  console.log(isFavorite, audioId, updatedAudio)
...
const getUserLastSession = async (req, res) => {
  ...
  let lastSession;
  try {
    const allSessions = await Session.find({ userId }).populate(
      "meditationAudio"
    );
